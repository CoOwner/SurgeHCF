...
public class ConquestTracker implements EventTracker, Listener
{
    ...
    public ConquestTracker(final SurgeCore ins) {
        ...
        for (final CaptureZone captureZone : captureZones) {
            ...
            if (remainingMillis <= 0L) {
                ...
                if (playerFaction != null) {
                    ...
                    Bukkit.broadcastMessage(ChatColor.YELLOW + "[" + eventFaction.getName() + "] " + ChatColor.LIGHT_PURPLE + playerFaction.getName() + ChatColor.GOLD + " gained " + 1 + " point for capturing " + captureZone.getDisplayName() + ChatColor.GOLD + ". " + ChatColor.AQUA + '(' + newPoints + '/' + 200 + ')');
                }
                return;
            }
    ...
    public void onPlayerDeath(final PlayerDeathEvent event) {
        ...
        if (currentEventFac instanceof ConquestFaction) {
            ...
            if (playerFaction != null) {
                ...
                event.setDeathMessage(ChatColor.YELLOW + "[" + currentEventFac.getName() + "] " + ChatColor.LIGHT_PURPLE + playerFaction.getName() + ChatColor.GOLD + " lost " + ChatColor.BOLD + Math.min(20, oldPoints) + ChatColor.GOLD + " points because " + player.getName() + " died." + ChatColor.AQUA + " (" + this.getPoints(playerFaction) + '/' + 200 + ')' + ChatColor.YELLOW + '.');
            }
        }
    }
