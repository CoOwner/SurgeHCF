...
import com.surgehcf.core.hcf.timer.TimerRunnable;
 ...
 public class EnderPearlTimer extends PlayerTimer implements org.bukkit.event.Listener
 {
   ...
   public void onProjectileLaunch(ProjectileLaunchEvent event) {
     ...
     if ((projectile instanceof EnderPearl)) {
       ...
       if ((source instanceof Player)) {
         ...
         if (remaining > 0L) {
           shooter.sendMessage(ChatColor.RED + "You still have a " + getDisplayName() + ChatColor.RED + " cooldown for another " + SurgeCore.getRemaining(remaining, true, false) + ChatColor.RED + '.');
   ...
   public void onInventoryDrag(InventoryDragEvent event) {
     ...
     Player player; PearlNameFaker pearlNameFaker; if ((humanEntity instanceof Player)) {
     ...
     public void run() {
       ...
       if ((stack != null) && (stack.getType() == Material.ENDER_PEARL)) {
         ...
         if (remaining > 0L) {
           ...
           item.c(ChatColor.YELLOW + "Enderpearl Cooldown: " + ChatColor.LIGHT_PURPLE + SurgeCore.getRemaining(remaining, true, true));
...
/* Location:              E:\Users\PUTTYDOTEXE\Desktop\Gabe\OLD\ZOlus\HCF (Zolus).jar!\com\exodon\hcf\timer\type\EnderPearlTimer.class
