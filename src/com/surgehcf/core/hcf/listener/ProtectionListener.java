...
public class ProtectionListener implements Listener
{
    ...
    public ProtectionListener(final SurgeCore plugin) {
        this.plugin = plugin;
    }
    ...
    public static boolean attemptBuild(final Entity entity, final Location location, final String denyMessage, final boolean isInteraction) {
        ...
        if (entity instanceof Player) {
            ...
            if (player != null && player.getWorld().getEnvironment() == World.Environment.THE_END) {
                player.sendMessage(ChatColor.YELLOW + "You cannot build in the end.");
                return false;
            }
            final Faction factionAt = SurgeCore.getPlugin().getFactionManager().getFactionAt(location);
            ...
            if (player != null && factionAt instanceof PlayerFaction) {
                final PlayerFaction playerFaction = SurgeCore.getPlugin().getFactionManager().getPlayerFaction(player);
                if (playerFaction != null && playerFaction.equals(factionAt)) {
                    result = true;
                }
            }
            if (result) {
                if (!isInteraction && Math.abs(location.getBlockX()) <= 500 && Math.abs(location.getBlockZ()) <= 500) {
                    if (denyMessage != null && player != null) {
                        player.sendMessage(ChatColor.YELLOW + "You cannot build within " + ChatColor.GREEN + 500 + ChatColor.YELLOW + " blocks from spawn.");
                    }
                    return false;
                }
            }
    ...
    public static boolean canBuildAt(final Location from, final Location to) {
        final Faction toFactionAt = SurgeCore.getPlugin().getFactionManager().getFactionAt(to);
        if (toFactionAt instanceof Raidable && !((Raidable)toFactionAt).isRaidable()) {
            final Faction fromFactionAt = SurgeCore.getPlugin().getFactionManager().getFactionAt(from);
    ...
    public void onPlayerTeleport(final PlayerTeleportEvent event) {
        if (event.getCause() == PlayerTeleportEvent.TeleportCause.ENDER_PEARL) {
            ...
            if (toFactionAt.isSafezone() && !this.plugin.getFactionManager().getFactionAt(event.getFrom()).isSafezone()) {
                final Player player = event.getPlayer();
                player.sendMessage(ChatColor.RED + "You may not enderpearl into a safezone. You have been refunded your pearl!");
    ...
    public void onPlayerPortal(final PlayerPortalEvent event) {
        if (event.getCause() == PlayerTeleportEvent.TeleportCause.NETHER_PORTAL) {
            ...
            if (fromFac.isSafezone()) {
                ...
                player.sendMessage(ChatColor.GREEN + "Teleported to spawn of target world since you were in a safezone!");
                return;
            }
            if (event.useTravelAgent() && to.getWorld().getEnvironment() == World.Environment.NORMAL) {
                ...
                if (factionAt instanceof ClaimableFaction) {
                    ...
                    player.sendMessage(ChatColor.YELLOW + "Portal would have created portal in territory of " + factionAt.getDisplayName((CommandSender)player) + ChatColor.YELLOW + ". Your portal was not created!");
                    event.setCancelled(true);
                }
            }
        }
            ...
            if (attacker != null) {
                ...
                if (playerFaction != null && (attackerFaction = this.plugin.getFactionManager().getPlayerFaction(attacker)) != null) {
                    ...
                    if (attackerFaction.equals(playerFaction)) {
                        attacker.sendMessage(CoreConfiguration.TEAMMATE_COLOUR + astrix + player.getName() + ChatColor.YELLOW + " is in your faction.");
                        event.setCancelled(true);
                    }
                    else if (attackerFaction.getAllied().contains(playerFaction.getUniqueID())) {
                        attacker.sendMessage(CoreConfiguration.ALLY_COLOUR + astrix + player.getName() + ChatColor.YELLOW + " is an ally.");
                        event.setCancelled(true);
                    }
                }
            }
        ...
        if (entered instanceof Player) {
            ...
            if (vehicle instanceof Horse) {
                ...
                if (owner != null && !owner.equals(entered)) {
                    ((Player)entered).sendMessage(ChatColor.GREEN + "You cannot use a Horse that belongs to " + ChatColor.RED + owner.getName() + ChatColor.YELLOW + '.');
                    event.setCancelled(true);
                }
            }
        }
