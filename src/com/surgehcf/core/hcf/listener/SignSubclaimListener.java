...
public class SignSubclaimListener implements Listener
{
    ...
    public SignSubclaimListener(final SurgeCore plugin) {
        this.plugin = plugin;
    }
        ...
        if (materialData instanceof Sign) {
            ...
            if (this.isSubclaimable(attatchedBlock)) {
                ...
                if (playerFaction.equals(factionAt)) {
                    ...
                    List actualMembers = memberList.stream().filter(member -> playerFaction.getMember(member) != null).collect(Collectors.toList());
                    playerFaction.broadcast(CoreConfiguration.TEAMMATE_COLOUR + player.getName() + ChatColor.YELLOW + " has created a subclaim on block type " + ChatColor.LIGHT_PURPLE + attatchedBlock.getType().toString() + ChatColor.YELLOW + " at " + ChatColor.WHITE + '[' + attatchedBlock.getX() + ", " + attatchedBlock.getZ() + ']' + ChatColor.YELLOW + " for " + (leaderChest ? "leaders" : (actualMembers.isEmpty() ? "captains" : ("members " + ChatColor.GRAY + '[' + ChatColor.DARK_GREEN + StringUtils.join((Iterable)actualMembers, ", ") + ChatColor.GRAY + ']'))));
                }
            }
        }
    }
    ...
    public void onInventoryMoveItem(final InventoryMoveItemEvent event) {
        ...
        if (holder instanceof Chest) {
            ...
            sourceBlocks = (Collection<Block>)Lists.newArrayList(new Block[] { ((Chest)doubleChest.getLeftSide()).getBlock(), ((Chest)doubleChest.getRightSide()).getBlock() });
        }
    ...
    static {
        SUBCLAIM_PREFIX = ChatColor.GOLD + ChatColor.BOLD.toString() + "- Subclaim -";
