package com.surgehcf.core.hcf.pvpclass.archer;
  ...
  implements Listener
{
  ...
  public ArcherClass(SurgeCore plugin)
  {
  ...
  public void onPlayerClickSugar(PlayerInteractEvent e)
  {
    ...
    {
      if (CooldownTimers.isOnCooldown("Archer_item_cooldown", p))
      {
        p.sendMessage(ChatColor.RED + "You are still on a cooldown for another: " + ChatColor.DARK_RED.toString() + CooldownTimers.getCooldownForPlayerInt("Archer_item_cooldown", p) + ChatColor.RED.toString() + " seconds");
        e.setCancelled(true);
        return;
      }
      CooldownTimers.addCooldown("Archer_item_cooldown", p, 25);
      ...
      p.addPotionEffect(new PotionEffect(PotionEffectType.SPEED, 120, 5));
      ...
      if ((source instanceof Player))
      {
        ...
        if (this.plugin.getTimerManager().archerTimer.getRemaining((Player)entity) == 0L)
        {
          ...
          for (Player player : Bukkit.getOnlinePlayers()) {
            this.plugin.getScoreboardHandler().getPlayerBoard(player.getUniqueId()).addUpdates(Arrays.asList(Bukkit.getOnlinePlayers()));
          }
