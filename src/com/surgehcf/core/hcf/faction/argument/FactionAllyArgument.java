...
import com.surgehcf.core.hcf.faction.type.PlayerFaction;
 ...
 {
   public FactionAllyArgument(SurgeCore plugin)
   {
   ...
   public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
     ...
     com.surgehcf.core.hcf.faction.type.Faction containingFaction = this.plugin.getFactionManager().getContainingFaction(args[1]);
     ...
     Collection<java.util.UUID> allied = playerFaction.getAllied();
     if (allied.size() >= 1) {
       sender.sendMessage(ChatColor.RED + "Your faction already has reached the alliance limit, which is " + 1 + '.');
       return true;
     }
     if (targetFaction.getAllied().size() >= 1) {
       sender.sendMessage(targetFaction.getDisplayName(sender) + ChatColor.RED + " has reached their maximum alliance limit, which is " + 1 + '.');
       return true;
     }
     ...
     if (targetFaction.getRequestedRelations().remove(playerFaction.getUniqueID()) != null) {
       com.surgehcf.core.hcf.faction.event.FactionRelationCreateEvent event = new com.surgehcf.core.hcf.faction.event.FactionRelationCreateEvent(playerFaction, targetFaction, RELATION);
     ...
     targetFaction.broadcast(playerFaction.getDisplayName(targetFaction) + ChatColor.YELLOW + " has sent a request to be " + RELATION.getDisplayName() + ChatColor.YELLOW + ". Use " + com.surgehcf.core.hcf.CoreConfiguration.ALLY_COLOUR + "/faction " + getName() + ' ' + playerFaction.getName() + ChatColor.YELLOW + " to accept.");
     return true;
   }
